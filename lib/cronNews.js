...
// les genera una imagen única con IA (FLUX vía NVIDIA NIM, mejor calidad que
// el motor anterior; si no está disponible o falla, respaldo en Pollinations
// y, en último caso, foto de stock real de Pexels), y guarda todo en Supabase.
//
...
//   NVIDIA_API_KEY             → gratis, sin tarjeta, en https://build.nvidia.com
//                                  (respaldo de reescritura Y motor principal de imágenes/FLUX)
//   PEXELS_API_KEY             → cuenta gratis en https://www.pexels.com/api (último respaldo)
...
//
// Requiere un bucket PÚBLICO llamado `imagenes` en Supabase Storage (para las
// imágenes generadas con FLUX — Pollinations.ai, el respaldo, sigue sin
// necesitar key ni storage propio, da su propia URL directa).
//
// Antes de usar esto hay que correr una vez sql/noticias.sql en el SQL
// Editor de Supabase para crear la tabla.

import { createHash } from 'node:crypto';
import { embedTitles, maxSimilarity } from './nvidiaEmbed.js';
import { secretoValido } from './timingSafeEqual.js';

// Cada categoría tiene un periodista fijo (perfil generado con IA, ver
// data/autores.json y equipo.html) — la firma que se guarda es siempre la
// misma para la misma categoría, no varía por artículo.
const AUTOR_POR_CATEGORIA = {
  ia: 'Mateo Rojas',
  videojuegos: 'Valentina Ibarra',
  deportes: 'Diego Salinas',
  salud: 'Antonia Muñoz',
  actualidad: 'Renato Fuentes',
};
...
// Checklist-10-etapas (auditoría 2026-08-05): Groq/NVIDIA no tenían timeout
// propio, dependían solo del corte duro de maxDuration en vercel.json.
const TEXT_TIMEOUT_MS = 25000;

function fetchConTimeout(url, opts, timeoutMs = TEXT_TIMEOUT_MS) {
...
// ─── IMÁGENES: FLUX (NVIDIA NIM) → Pollinations → Pexels ──────────────────
//
// FLUX (black-forest-labs/flux.1-dev, gratis vía NVIDIA NIM) da mucha mejor
// calidad que Pollinations — mismo diagnóstico y mismo fix que ya se probó a
// fondo en scripts/video-pipeline/. Diferencia técnica: Pollinations devuelve
// una URL pública directa (el sitio solo la referencia), FLUX devuelve la
// imagen en base64 — hay que subirla a algo con URL pública, acá se usa
// Supabase Storage (mismo proyecto que ya se usa para la base de datos, sin
// cuenta nueva). Requiere un bucket público llamado `imagenes` ya creado.
//
// Cadena de resiliencia: FLUX (con reintento + respaldo si el filtro de
// contenido bloquea el prompt) → Pollinations si FLUX no está disponible o
// agotó los reintentos → Pexels (foto de stock real) como último recurso.

const FLUX_URL = 'https://ai.api.nvidia.com/v1/genai/black-forest-labs/flux.1-dev';
// Prompt de respaldo neutro — mismo usado en el pipeline de video, ya
// comprobado que no dispara CONTENT_FILTERED.
const FLUX_PROMPT_RESPALDO = 'Abstract editorial illustration, soft gradient background, no text, no faces, professional news photography style';
  ...
  try {
    const resp = await fetch(FLUX_URL, {
      ...
      body: JSON.stringify({
        ...
        cfg_scale: 3.5,
        width: 1216,
        height: 832, // el ratio horizontal más cercano a 1200x675 que soporta FLUX
        steps: 30,
...
async function subirImagenASupabase(base64, supabaseUrl, serviceKey) {
  const nombre = `${Date.now()}-${Math.floor(Math.random() * 1e6)}.jpg`;
  const resp = await fetch(`${supabaseUrl}/storage/v1/object/imagenes/${nombre}`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${serviceKey}`,
  ...
  if (!resp.ok) throw new Error(`Supabase Storage HTTP ${resp.status}: ${await resp.text()}`);
  return `${supabaseUrl}/storage/v1/object/public/imagenes/${nombre}`;
}

async function tryFlux(prompt, supabaseUrl, serviceKey) {
  if (!process.env.NVIDIA_API_KEY) return null;
  try {
    let base64;
    try {
      base64 = await llamarFlux(prompt);
    } catch (err) {
      if (err.contentFiltered) {
        console.error('FLUX bloqueó el prompt (CONTENT_FILTERED), uso respaldo genérico:', prompt);
        base64 = await llamarFlux(FLUX_PROMPT_RESPALDO);
      } else {
        throw err;
      }
    }
    return await subirImagenASupabase(base64, supabaseUrl, serviceKey);
  } catch (err) {
    console.error('Intento de FLUX falló:', err.message || err);
    return null;
  }
}
...
async function findImage(imagenPrompt, fallbackQuery, titulo, supabaseUrl, serviceKey) {
  // Si el modelo no devolvió imagen_prompt (JSON truncado, campo omitido, etc.),
  // NUNCA le mandamos al generador el fallbackQuery genérico a secas: eso es
  ...
  if (prompt) {
    const fluxUrl = await tryFlux(prompt, supabaseUrl, serviceKey);
    if (fluxUrl) return { imagen_url: fluxUrl, imagen_credito: 'Generada con IA' };
    console.error('FLUX no disponible/agotado, uso Pollinations de respaldo.');
...
export async function runNews(req, res) {
  const authHeader = req.headers.authorization;
  const isVercelCron = secretoValido(authHeader, `Bearer ${process.env.CRON_SECRET}`);
  const providedKey = req.query.key;
  const isManualTrigger = secretoValido(providedKey, process.env.ADMIN_KEY);
  ...
  await runWithConcurrency(worklist, CONCURRENCY, async ({ cat, article }) => {
    ...
    try {
      ...
      const { imagen_url, imagen_credito } = await limitImageCalls(() => findImage(rewritten.imagen_prompt, cat.imageFallbackQuery, rewritten.titulo, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY));
      ...
      const wasInserted = await saveNoticia(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
        ...
        autor: AUTOR_POR_CATEGORIA[cat.categoria] || null,
